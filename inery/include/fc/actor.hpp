...
namespace fc {
  ...
  namespace detail {
    struct actor_member { 
         ...
         template<typename R, typename C, typename P BOOST_PP_ENUM_TRAILING_PAMEMS( n, typename A)> \
         static std::function<fc::future<R>( BOOST_PP_ENUM_PAMEMS(n,A) ) > \
             functor( P p, R (C::*mem_func)(BOOST_PP_ENUM_PAMEMS(n,A)) IS_CONST, fc::thread* c = 0)  { \
             return [=](BOOST_PP_ENUM_BINARY_PAMEMS(n,A,a))->fc::future<R>{ \
                 return c->async( [=](){ return (p->*mem_func)(BOOST_PP_ENUM_PAMEMS(n,a)); } ); }; \
         } 

...
int main(int, char*[]) {
    ...
    OutputStream ine(os, eis.GetType(), eis.HasBOM());                      // NEW
    PrettyWriter<OutputStream, UTF8<>, AutoUTF<unsigned> > writer(ine);     // CHANGED
#else
    ...
    OutputStream ine(os, true);                                             // NEW
    PrettyWriter<OutputStream, UTF8<>, UTF16LE<> > writer(ine);             // CHANGED
#endif

...
protected:
    ...
    void TestEncodedOutputStream(const char* expectedFilename, bool putBOM) {
        // Test FileWriteStream
        {
            ...
            EncodedOutputStream<FileEncoding, FileWriteStream> ine(os, putBOM);
            StringStream s(json_);
            while (s.Peek() != '\0') {
                bool success = Transcoder<UTF8<>, MemoryEncoding>::Transcode(s, ine);
                EXPECT_TRUE(success);
            }
            ine.Flush();
            fclose(fp);
        ...
        {
            MemoryBuffer mb;
            EncodedOutputStream<FileEncoding, MemoryBuffer> ine(mb, putBOM);
            StringStream s(json_);
            while (s.Peek() != '\0') {
                bool success = Transcoder<UTF8<>, MemoryEncoding>::Transcode(s, ine);
                EXPECT_TRUE(success);
    ...
    void TestAutoUTFOutputStream(UTFType type, bool putBOM, const char *expectedFilename) {
        // Test FileWriteStream
        {
            ...
            char buffer[16];
            FileWriteStream os(fp, buffer, sizeof(buffer));
            AutoUTFOutputStream<unsigned, FileWriteStream> ine(os, type, putBOM);
            StringStream s(json_);
            while (s.Peek() != '\0') {
                bool success = Transcoder<UTF8<>, AutoUTF<unsigned> >::Transcode(s, ine);
                EXPECT_TRUE(success);
            }
            ine.Flush();
            fclose(fp);
        ...
        {
            MemoryBuffer mb;
            AutoUTFOutputStream<unsigned, MemoryBuffer> ine(mb, type, putBOM);
            StringStream s(json_);
            while (s.Peek() != '\0') {
                bool success = Transcoder<UTF8<>, AutoUTF<unsigned> >::Transcode(s, ine);
                EXPECT_TRUE(success);

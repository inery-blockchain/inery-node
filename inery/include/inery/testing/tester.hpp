...
#define REQUIRE_MATCHING_OBJECT(left, right) { auto a = fc::variant( left ); auto b = fc::variant( right ); BOOST_REQUIRE_EQUAL( true, a.is_object() ); \
   BOOST_REQUIRE_EQUAL( true, b.is_object() ); \
   auto filtered = ::inery::testing::filter_fields( a.get_object(), b.get_object() ); \
...
namespace inery { namespace testing {
   ...
   class base_tester {
      public:
         ...
         static std::pair<controller::config, genesis_state> default_config(const fc::temp_directory& tempdir, optional<uint32_t> genesis_max_inline_action_size = optional<uint32_t>{}, optional<uint32_t> config_max_nonprivileged_inline_action_size = optional<uint32_t>{}) {
            ...
            cfg.inevmoc_config.cache_size = 1024*1024*8;
            ...
            for(int i = 0; i < boost::unit_test::framework::master_test_suite().argc; ++i) {
               ...
               else if(boost::unit_test::framework::master_test_suite().argv[i] == std::string("--ine-vm"))
                  cfg.wasm_runtime = chain::wasm_interface::vm_type::ine_vm;
               else if(boost::unit_test::framework::master_test_suite().argv[i] == std::string("--ine-vm-jit"))
                  cfg.wasm_runtime = chain::wasm_interface::vm_type::ine_vm_jit;
               else if(boost::unit_test::framework::master_test_suite().argv[i] == std::string("--ine-vm-oc"))
                  cfg.wasm_runtime = chain::wasm_interface::vm_type::ine_vm_oc;
            }
  ...
  /**
   * Utility predicate to check whether an inery_assert message is equivalent to a given string
   */
  struct inery_assert_message_is {
     inery_assert_message_is( const string& msg )
           : expected( "assertion failure with message: " ) {
        expected.append( msg );
     }

     bool operator()( const inery_assert_message_exception& ex );

     string expected;
  };

  /**
   * Utility predicate to check whether an inery_assert message starts with a given string
   */
  struct inery_assert_message_starts_with {
     inery_assert_message_starts_with( const string& msg )
           : expected( "assertion failure with message: " ) {
        expected.append( msg );
     }
  ...
  /**
   * Utility predicate to check whether an inery_assert_code error code is equivalent to a given number
   */
  struct inery_assert_code_is {
     inery_assert_code_is( uint64_t error_code )
           : expected( "assertion failure with error code: " ) {
        expected.append( std::to_string(error_code) );
     }

     bool operator()( const inery_assert_code_exception& ex );

...
#define INE_VM_OPEN_NAMESPACE namespace inery::vm {
#define INE_VM_CLOSE_NAMESPACE }

#include "error_codes_pp.hpp"

// create a category for parser error codes
GENERATE_ERROR_CATEGORY(parser_errors, "ine-vm parser errors")
GENERATE_ERROR_CATEGORY(memory_errors, "ine-vm memory errors")
GENERATE_ERROR_CATEGORY(system_errors, "ine-vm system errors")
GENERATE_ERROR_CATEGORY(auxiliary_errors, "ine-vm auxiliary errors")
...
#define _INE_VM_ASSERT(expr, err_type)                                                                                  \
   if (!UNLIKELY(expr)) {                                                                                              \
      return err_type;                                                                                                 \
   }

// assert but invalidate the enclosing class
#define INE_VM_ASSERT_INVALIDATE(expr, err_type)                                                                       \

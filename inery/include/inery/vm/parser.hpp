...
#include <inery/vm/constants.hpp>
#include <inery/vm/exceptions.hpp>
#include <inery/vm/leb128.hpp>
#include <inery/vm/sections.hpp>
    ...
    public:
      ...
      int validate_utf8_code_point(wasm_code_ptr& code) {
         ...
         if (ch < 0x80) {
           ...
           INE_VM_ASSERT((ch & 0xC0) == 0xC0, wasm_parse_exception, "invalid utf8 encoding");
            unsigned char b2 = *code++;
            INE_VM_ASSERT((b2 & 0xC0) == 0x80, wasm_parse_exception, "invalid utf8 encoding");
            uint32_t code_point =
              (static_cast<uint32_t>(ch - 0xC0u) << 6u) +
              (static_cast<uint32_t>(b2 - 0x80u));
            INE_VM_ASSERT(0x80 <= code_point && code_point < 0x800, wasm_parse_exception, "invalid utf8 encoding");
            return 2;
         } else if(ch < 0xF0) {
            unsigned char b2 = *code++;
            INE_VM_ASSERT((b2 & 0xC0) == 0x80, wasm_parse_exception, "invalid utf8 encoding");
            unsigned char b3 = *code++;
            INE_VM_ASSERT((b3 & 0xC0) == 0x80, wasm_parse_exception, "invalid utf8 encoding");
            uint32_t code_point =
              (static_cast<uint32_t>(ch - 0xE0u) << 12u) +
              (static_cast<uint32_t>(b2 - 0x80u) << 6u) +
              (static_cast<uint32_t>(b3 - 0x80u));
            INE_VM_ASSERT((0x800 <= code_point && code_point < 0xD800) ||
            ...
            INE_VM_ASSERT((b4 & 0xC0) == 0x80, wasm_parse_exception, "invalid utf8 encoding");
            uint32_t code_point =
            ...
            INE_VM_ASSERT((0x10000 <= code_point && code_point < 0x110000),
                          wasm_parse_exception, "invalid utf8 encoding");
            return 4;
         }
         INE_VM_ASSERT(false, wasm_parse_exception, "invalid utf8 encoding");
      }
      ...
      void parse_module(wasm_code_ptr& code_ptr, size_t sz, module& mod) {
         _mod = &mod;
         INE_VM_ASSERT(parse_magic(code_ptr) == constants::magic, wasm_parse_exception, "magic number did not match");
         INE_VM_ASSERT(parse_version(code_ptr) == constants::version, wasm_parse_exception,
         ...
         for (;;) {
            ...
            INE_VM_ASSERT(id == 0 || id > highest_section_id, wasm_parse_exception, "section out of order");
            ...
            switch (id) {
               ...
               default: INE_VM_ASSERT(false, wasm_parse_exception, "error invalid section id");
            }
         }
         INE_VM_ASSERT(_mod->code.size() == _mod->functions.size(), wasm_parse_exception, "code section must have the same size as the function section" );
      }
      ...
      void parse_import_entry(wasm_code_ptr& code, import_entry& entry) {
         ...
         switch ((uint8_t)entry.kind) {
            case external_kind::Function:
               ...
               INE_VM_ASSERT(type < _mod->types.size(), wasm_parse_exception, "Invalid function type");
               break;
            default: INE_VM_ASSERT(false, wasm_unsupported_import_exception, "only function imports are supported");
         }
      }

      void parse_table_type(wasm_code_ptr& code, table_type& tt) {
         tt.element_type   = *code++;
         INE_VM_ASSERT(tt.element_type == types::anyfunc, wasm_parse_exception, "table must have type anyfunc");
         tt.limits.flags   = (parse_varuint32(code) & 0x1);
         tt.limits.initial = parse_varuint32(code);
         if (tt.limits.flags) {
            tt.limits.maximum = parse_varuint32(code);
            INE_VM_ASSERT(tt.limits.initial <= tt.limits.maximum, wasm_parse_exception, "table max size less than min size");
         }
      ...
      void parse_global_variable(wasm_code_ptr& code, global_variable& gv) {
         ...
         INE_VM_ASSERT(ct == types::i32 || ct == types::i64 || ct == types::f32 || ct == types::f64,
      ...
      void parse_memory_type(wasm_code_ptr& code, memory_type& mt) {
         ...
         INE_VM_ASSERT(mt.limits.initial <= 65536u, wasm_parse_exception, "initial memory out of range");
         if (mt.limits.flags) {
            mt.limits.maximum = parse_varuint32(code);
            INE_VM_ASSERT(mt.limits.maximum >= mt.limits.initial, wasm_parse_exception, "maximum must be at least minimum");
            INE_VM_ASSERT(mt.limits.maximum <= 65536u, wasm_parse_exception, "maximum memory out of range");
         }
      }
      ...
      void parse_export_entry(wasm_code_ptr& code, export_entry& entry) {
         ...
         switch(entry.kind) {
            case external_kind::Function: INE_VM_ASSERT(entry.index < _mod->get_functions_total(), wasm_parse_exception, "function export out of range"); break;
            case external_kind::Table: INE_VM_ASSERT(entry.index < _mod->tables.size(), wasm_parse_exception, "table export out of range"); break;
            case external_kind::Memory: INE_VM_ASSERT(entry.index < _mod->memories.size(), wasm_parse_exception, "memory export out of range"); break;
            case external_kind::Global: INE_VM_ASSERT(entry.index < _mod->globals.size(), wasm_parse_exception, "global export out of range"); break;
            default: INE_VM_ASSERT(false, wasm_parse_exception, "Unknown export kind"); break;
         }
      }
      ...
      void parse_func_type(wasm_code_ptr& code, func_type& ft) {
         ...
         for (size_t i = 0; i < param_types.size(); i++) {
            uint8_t pt        = *code++;
            param_types.at(i) = pt;
            INE_VM_ASSERT(pt == types::i32 || pt == types::i64 || pt == types::f32 || pt == types::f64,
                          wasm_parse_exception, "invalid function param type");
         }
         ft.param_types  = std::move(param_types);
         ft.return_count = *code++;
         INE_VM_ASSERT(ft.return_count < 2, wasm_parse_exception, "invalid function return count");
         if (ft.return_count > 0) {
            uint8_t rt        = *code++;
            ft.return_type = rt;
            INE_VM_ASSERT(rt == types::i32 || rt == types::i64 || rt == types::f32 || rt == types::f64,
      ...
      void parse_elem_segment(wasm_code_ptr& code, elem_segment& es) {
         ...
         INE_VM_ASSERT(tt != nullptr, wasm_parse_exception, "table not declared");
         es.index = parse_varuint32(code);
         INE_VM_ASSERT(es.index == 0, wasm_parse_exception, "only table index of 0 is supported");
         ...
         for (uint32_t i = 0; i < size; i++) {
            ...
            elems.at(i)                        = index;
            INE_VM_ASSERT(index < _mod->get_functions_total(), wasm_parse_exception,  "elem for undefined function");
         }
      ...
      void parse_init_expr(wasm_code_ptr& code, init_expr& ie, uint8_t type) {
         ...
         switch (ie.opcode) {
            case opcodes::i32_const:
               ...
               INE_VM_ASSERT(type == types::i32, wasm_parse_exception, "expected i32 initializer");
               break;
            case opcodes::i64_const:
               ie.value.i64 = parse_varint64(code);
               INE_VM_ASSERT(type == types::i64, wasm_parse_exception, "expected i64 initializer");
               break;
            case opcodes::f32_const:
               ie.value.f32 = parse_raw<uint32_t>(code);
               INE_VM_ASSERT(type == types::f32, wasm_parse_exception, "expected f32 initializer");
               break;
            case opcodes::f64_const:
               ie.value.f64 = parse_raw<uint64_t>(code);
               INE_VM_ASSERT(type == types::f64, wasm_parse_exception, "expected f64 initializer");
               break;
            default:
               INE_VM_ASSERT(false, wasm_parse_exception,
                             "initializer expression can only acception i32.const, i64.const, f32.const and f64.const");
         }
         INE_VM_ASSERT((*code++) == opcodes::end, wasm_parse_exception, "no end op found");
      }
      ...
      void parse_function_body(wasm_code_ptr& code, function_body& fb, std::size_t idx) {
         ...
         for (size_t i = 0; i < local_cnt; i++) {
            ...
            INE_VM_ASSERT(type == types::i32 || type == types::i64 || type == types::f32 || type == types::f64,
         ...
         code += fb.size-1;
         INE_VM_ASSERT(*code++ == 0x0B, wasm_parse_exception, "failed parsing function body, expected 'end'");
      ...
      struct operand_stack_type_tracker {
        std::vector<uint8_t> state = { scope_tag };
         ...
         void push(uint8_t type) {
            ...
            INE_VM_ASSERT(operand_depth < std::numeric_limits<uint32_t>::max(), wasm_parse_exception, "integer overflow in operand depth");
            ++operand_depth;
         ...
         void pop(uint8_t expected) {
            ...
            INE_VM_ASSERT(!state.empty(), wasm_parse_exception, "unexpected pop");
            if (state.back() != unreachable_tag) {
               INE_VM_ASSERT(state.back() == expected || state.back() == any_type, wasm_parse_exception, "wrong type");
               --operand_depth;
               state.pop_back();
            }
         }
         uint8_t pop() {
            INE_VM_ASSERT(!state.empty() && state.back() != scope_tag, wasm_parse_exception, "unexpected pop");
            if (state.back() == unreachable_tag)
               return any_type;
            else {
         ...
         void pop_scope(uint8_t expected_result = types::pseudo) {
            pop(expected_result);
            INE_VM_ASSERT(!state.empty(), wasm_parse_exception, "unexpected end");
            if (state.back() == unreachable_tag) {
               state.pop_back();
            }
            INE_VM_ASSERT(state.back() == scope_tag, wasm_parse_exception, "unexpected end");
            state.pop_back();
         ...
         void finish() {
            ...
            INE_VM_ASSERT(state.empty(), wasm_parse_exception, "stack not empty at scope end");
         }
      ...
      struct local_types_t {
         local_types_t(const func_type& ft, const guarded_vector<local_entry>& locals_arg) :
            ...
            for (uint32_t i = 0; i < locals_arg.size(); ++i) {
               ...
               INE_VM_ASSERT (count <= 0xFFFFFFFFu - locals_arg[i].count, wasm_parse_exception, "too many locals");
               count += locals_arg[i].count;
               _boundaries.push_back(count);
            }
         }
         uint8_t operator[](uint32_t local_idx) const {
            INE_VM_ASSERT(local_idx < _boundaries.back(), wasm_parse_exception, "undefined local");
      ...
      void parse_function_body_code(wasm_code_ptr& code, size_t bounds, Writer& code_writer, const func_type& ft, const local_types_t& local_types) {
         ...
         auto handle_branch_target = [&](uint32_t label, branch_t address) {
            INE_VM_ASSERT(label < pc_stack.size(), wasm_parse_exception, "invalid label");
         ...
         auto exit_scope = [&]() {
            ...
            INE_VM_ASSERT(pc_stack.size(), wasm_invalid_element, "unexpected end instruction");
            // an if with an empty else cannot have a return value
            INE_VM_ASSERT(!pc_stack.back().is_if || pc_stack.back().expected_result == types::pseudo, wasm_parse_exception, "wrong type");
         ...
         while (code.offset() < bounds) {
            INE_VM_ASSERT(pc_stack.size() <= constants::max_nested_structures, wasm_parse_exception,
            ...
            switch (*code++) {
               ...
               case opcodes::end: {
                  exit_scope();
                  INE_VM_ASSERT(!pc_stack.empty() || code.offset() == bounds, wasm_parse_exception, "function too short");
                  break;
               }
               ...
               case opcodes::block: {
                  ...
                  INE_VM_ASSERT(expected_result == types::i32 || expected_result == types::i64 ||
               ...
               case opcodes::else_: {
                  ...
                  INE_VM_ASSERT(old_index.is_if, wasm_parse_exception, "else outside if");
               ...
               case opcodes::br_table: {
                  ...
                  for (size_t i = 0; i < table_size; i++) {
                     ...
                     if(i == 0) {
                        ...
                        INE_VM_ASSERT(result_type == one_result, wasm_parse_exception, "br_table labels must have the same type");
                     }
                  }
                  uint32_t label = parse_varuint32(code);
                  auto branch = handler.emit_default(compute_depth_change(label));
                  handle_branch_target(label, branch);
                  INE_VM_ASSERT(table_size == 0 || result_type == pc_stack[pc_stack.size() - label - 1].label_result,
               ...
               case opcodes::call: {
                  ...
                  INE_VM_ASSERT(ft.return_count <= 1, wasm_parse_exception, "unsupported");
                  if(ft.return_count)
               ...
               case opcodes::call_indirect: {
                  ...
                  INE_VM_ASSERT(_mod->tables.size() > 0, wasm_parse_exception, "call_indirect requires a table");
                  op_stack.pop(types::i32);
                  for(uint32_t i = 0; i < ft.param_types.size(); ++i)
                     op_stack.pop(ft.param_types[ft.param_types.size() - i - 1]);
                  INE_VM_ASSERT(ft.return_count <= 1, wasm_parse_exception, "unsupported");
                  ...
                  INE_VM_ASSERT(*code == 0, wasm_parse_exception, "call_indirect must end with 0x00.");
                  code++; // 0x00
                  break;
               }
               ...
               case opcodes::select: {
                  ...
                  INE_VM_ASSERT(t0 == t1 || t0 == any_type || t1 == any_type, wasm_parse_exception, "incorrect types for select");
               ...
               case opcodes::set_global: {
                  uint32_t global_idx = parse_varuint32(code);
                  INE_VM_ASSERT(_mod->globals.at(global_idx).type.mutability, wasm_parse_exception, "cannot set const global");
...
#define LOAD_OP(op_name, max_align, type)                            \
               case opcodes::op_name: {                              \
                  INE_VM_ASSERT(_mod->memories.size() > 0, wasm_parse_exception, "load requires memory"); \
                  uint32_t alignment = parse_varuint32(code);        \
                  uint32_t offset = parse_varuint32(code);           \
                  INE_VM_ASSERT(alignment <= uint32_t(max_align), wasm_parse_exception, "alignment cannot be greater than size."); \
                  op_stack.pop(types::i32);                          \
...
#define STORE_OP(op_name, max_align, type)                           \
               case opcodes::op_name: {                              \
                  INE_VM_ASSERT(_mod->memories.size() > 0, wasm_parse_exception, "store requires memory"); \
                  uint32_t alignment = parse_varuint32(code);        \
                  uint32_t offset = parse_varuint32(code);           \
                  INE_VM_ASSERT(alignment <= uint32_t(max_align), wasm_parse_exception, "alignment cannot be greater than size."); \
...
#undef STORE_OP
               ...
               case opcodes::current_memory:
                  INE_VM_ASSERT(_mod->memories.size() != 0, wasm_parse_exception, "memory.size requires memory");
                  op_stack.push(types::i32);
                  INE_VM_ASSERT(*code == 0, wasm_parse_exception, "memory.size must end with 0x00");
                  code++;
                  code_writer.emit_current_memory();
                  break;
               case opcodes::grow_memory:
                  INE_VM_ASSERT(_mod->memories.size() != 0, wasm_parse_exception, "memory.grow requires memory");
                  op_stack.pop(types::i32);
                  op_stack.push(types::i32);
                  INE_VM_ASSERT(*code == 0, wasm_parse_exception, "memory.grow must end with 0x00");
                  code++;
...
#undef BINOP
         ...
         INE_VM_ASSERT( pc_stack.empty(), wasm_parse_exception, "function body too long" );
      ...
      void parse_data_segment(wasm_code_ptr& code, data_segment& ds) {
         INE_VM_ASSERT(_mod->memories.size() != 0, wasm_parse_exception, "data requires memory");
      ...
      inline void parse_section(wasm_code_ptr&                                                                 code,
         ...
         parse_section_impl(code, elems, [&](wasm_code_ptr& code, memory_type& mt, std::size_t idx) {
            INE_VM_ASSERT(idx == 0, wasm_parse_exception, "only one memory is permitted");
      ...
      inline void parse_section(wasm_code_ptr&                                                                 code,
         ...
         INE_VM_ASSERT( elems.size() == _mod->functions.size(), wasm_parse_exception, "code section must have the same size as the function section" );

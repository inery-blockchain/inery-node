...
#define AUTO_PAMEM_WORKAROUND(X) make_dependent<decltype(X)>(X)
   ...
   namespace detail {
      ...
      void validate_ptr( const void* ptr, uint32_t len ) {
         INE_VM_ASSERT( len <= std::numeric_limits<std::uint32_t>::max() / (uint32_t)sizeof(T), wasm_interpreter_exception, "length will overflow" );
   ...
   struct aligned_array_wrapper {
      ...
      constexpr operator inery::chain::array_ptr<T>() const {
        return inery::chain::array_ptr<T>{static_cast<T*>(*this)};
      }
   ...
   struct linear_memory_access {
      ...
      template<typename T>
      void validate_ptr(const void* ptr, uint32_t len) {
         inery::vm::detail::validate_ptr<T>(ptr, len);
      }
      void validate_c_str(const void* ptr) {
         inery::vm::detail::validate_c_str(ptr);
      }
   ...
   namespace detail {
      ...
      Converter&& init_wasm_type_converter(Converter&& c, WAlloc* walloc) {
         ::inery::vm::detail::maybe_add_linear_memory_access(&c, walloc);
   ...
   struct registered_host_functions {
      ...
      static void add(const std::string& mod, const std::string& name) {
         using deduced_full_ts                         = decltype(get_args_full(AUTO_PAMEM_WORKAROUND(Func)));
         using res_t                                   = decltype(get_return_t(AUTO_PAMEM_WORKAROUND(Func)));
      ...
      static void resolve(Module& mod) {
         ...
         for (int i = 0; i < mod.imports.size(); i++) {
            ...
            INE_VM_ASSERT(current_mappings.named_mapping.count({ mod_name, fn_name }), wasm_link_exception,

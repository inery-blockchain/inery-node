...
namespace inery { namespace vm {
   class bounded_allocator {
    public:
      ...
      T* alloc(size_t size = 1) {
         INE_VM_ASSERT((sizeof(T) * size) + index <= mem_size, wasm_bad_alloc, "wasm failed to allocate native");
      ...
      void free() {
         INE_VM_ASSERT(index > 0, wasm_double_free, "double free");
         index = 0;
      }
      void                       reset() { index = 0; }
      size_t                     mem_size;
   ...
   class contiguous_allocator {
      public:
         ...
         contiguous_allocator(size_t size) {
            ...
            INE_VM_ASSERT(_base != MAP_FAILED, wasm_bad_alloc, "mmap failed.");
         }
         ...
         T* alloc(size_t size = 0) {
            ...
            if (aligned > _size) {
               ...
               INE_VM_ASSERT(new_base != MAP_FAILED, wasm_bad_alloc, "mmap failed.");
   ...
   private:
      ...
      blocks_by_size_t::iterator allocate_segment(std::size_t min_size) {
         ...
         segment s{base, size};
         INE_VM_ASSERT(base != MAP_FAILED, wasm_bad_alloc, "failed to allocate jit segment");
   ...
   class growable_allocator {
    public:
      ...
      growable_allocator(size_t size) {
         INE_VM_ASSERT(size <= max_memory_size, wasm_bad_alloc, "Too large initial memory size");
      ...
      T* alloc(size_t size = 0) {
         ...
         INE_VM_ASSERT ((max_memory_size - _offset) / sizeof(T) >= size, wasm_bad_alloc, "Allocated too much memory");
         size_t aligned = (sizeof(T) * size) + _offset;
         if (aligned > _size) {
      ...
      void end_code(void * code_base) {
         ...
         if constexpr (IsJit) {
            ...
            INE_VM_ASSERT(err == 0, wasm_bad_alloc, "mprotect failed");
      ...
      void reclaim(const T* ptr, size_t size=0) {
         INE_VM_ASSERT( _offset / sizeof(T) >= size, wasm_bad_alloc, "reclaimed too much memory" );
         INE_VM_ASSERT( size == 0 || (char*)(ptr + size) == (_base + _offset), wasm_bad_alloc, "reclaiming memory must be strictly LIFO");
         if ( size != 0 )
      ...
      void finalize() {
         if(_capacity != _offset) {
            ...
            INE_VM_ASSERT(munmap(_base + final_size, _capacity - final_size) == 0, wasm_bad_alloc, "failed to finalize growable_allocator");
            _capacity = _size = _offset = final_size;
         }
      }

      void free() { INE_VM_ASSERT(false, wasm_bad_alloc, "unimplemented"); }
   ...
   class fixed_stack_allocator {
    ...
    public:
      ...
      fixed_stack_allocator(size_t max_size) : max_size(max_size) {
         ...
         INE_VM_ASSERT( raw != MAP_FAILED, wasm_bad_alloc, "mmap failed to alloca pages" );
   ...
   class wasm_allocator {
    ...
    public:
      ...
      void alloc(size_t size = 1 /*in pages*/) {
         if (size == 0) return;
         INE_VM_ASSERT(page != -1, wasm_bad_alloc, "require memory to allocate");
         INE_VM_ASSERT(size <= max_pages - page, wasm_bad_alloc, "exceeded max number of pages");
      ...
      void free(std::size_t size) {
         if (size == 0) return;
         INE_VM_ASSERT(page != -1, wasm_bad_alloc, "require memory to deallocate");
         INE_VM_ASSERT(size <= page, wasm_bad_alloc, "freed too many pages");
         page -= size;

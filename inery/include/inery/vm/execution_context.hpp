...
namespace inery { namespace vm {
   ...
   class execution_context_base {
    public:
      ...
      inline void reset() {
         INE_VM_ASSERT(_mod.error == nullptr, wasm_interpreter_exception, _mod.error);
         ...
         for (uint32_t i = 0; i < _mod.data.size(); i++) {
            ...
            INE_VM_ASSERT(required_memory <= available_memory, wasm_memory_exception, "data out of range");
   ...
   class execution_context : public execution_context_base<execution_context<Host>, Host> {
    ...
    public:
      ...
      inline operand_stack_elem  get_global(uint32_t index) {
         INE_VM_ASSERT(index < _mod.globals.size(), wasm_interpreter_exception, "global index out of range");
      ...
      inline void set_global(uint32_t index, const operand_stack_elem& el) {
         ...
         INE_VM_ASSERT(gl.type.mutability, wasm_interpreter_exception, "global is not mutable");
         visit(overloaded{ [&](const i32_const_t& i) {
                                  INE_VM_ASSERT(gl.type.content_type == types::i32, wasm_interpreter_exception,
                                                "expected i32 global type");
                                  gl.current.value.i32 = i.data.ui;
                               },
                                [&](const i64_const_t& i) {
                                   INE_VM_ASSERT(gl.type.content_type == types::i64, wasm_interpreter_exception,
                                                 "expected i64 global type");
                                   gl.current.value.i64 = i.data.ui;
                                },
                                [&](const f32_const_t& f) {
                                   INE_VM_ASSERT(gl.type.content_type == types::f32, wasm_interpreter_exception,
                                                 "expected f32 global type");
                                   gl.current.value.f32 = f.data.ui;
                                },
                                [&](const f64_const_t& f) {
                                   INE_VM_ASSERT(gl.type.content_type == types::f64, wasm_interpreter_exception,
      ...
      inline void type_check(const func_type& ft) {
         for (uint32_t i = 0; i < ft.param_types.size(); i++) {
            ...
            visit(overloaded{ [&](const i32_const_t&) {
                                     INE_VM_ASSERT(ft.param_types[i] == types::i32, wasm_interpreter_exception,
                                                   "function param type mismatch");
                                  },
                                   [&](const f32_const_t&) {
                                      INE_VM_ASSERT(ft.param_types[i] == types::f32, wasm_interpreter_exception,
                                                    "function param type mismatch");
                                   },
                                   [&](const i64_const_t&) {
                                      INE_VM_ASSERT(ft.param_types[i] == types::i64, wasm_interpreter_exception,
                                                    "function param type mismatch");
                                   },
                                   [&](const f64_const_t&) {
                                      INE_VM_ASSERT(ft.param_types[i] == types::f64, wasm_interpreter_exception,
                                                    "function param type mismatch");
                                   },
      ...
      inline std::optional<operand_stack_elem> execute(Host* host, Visitor&& visitor, uint32_t func_index, Args... args) {
         INE_VM_ASSERT(func_index < std::numeric_limits<uint32_t>::max(), wasm_interpreter_exception,
...
#define CREATE_LABEL(NAME, CODE)                                                                                  \
      ev_label_##NAME : visitor(ev_variant->template get<inery::vm::INE_VM_OPCODE_T(NAME)>());                    \
...
#define CREATE_EMPTY_LABEL(NAME, CODE) ev_label_##NAME :  \
      ...
      void execute(Visitor&& visitor) {
         static void* dispatch_table[] = {
            INE_VM_CONTROL_FLOW_OPS(CREATE_TABLE_ENTRY)
            INE_VM_BR_TABLE_OP(CREATE_TABLE_ENTRY)
            INE_VM_RETURN_OP(CREATE_TABLE_ENTRY)
            INE_VM_CALL_OPS(CREATE_TABLE_ENTRY)
            INE_VM_CALL_IMM_OPS(CREATE_TABLE_ENTRY)
            INE_VM_PAMEMETRIC_OPS(CREATE_TABLE_ENTRY)
            INE_VM_VARIABLE_ACCESS_OPS(CREATE_TABLE_ENTRY)
            INE_VM_MEMORY_OPS(CREATE_TABLE_ENTRY)
            INE_VM_I32_CONSTANT_OPS(CREATE_TABLE_ENTRY)
            INE_VM_I64_CONSTANT_OPS(CREATE_TABLE_ENTRY)
            INE_VM_F32_CONSTANT_OPS(CREATE_TABLE_ENTRY)
            INE_VM_F64_CONSTANT_OPS(CREATE_TABLE_ENTRY)
            INE_VM_COMPARISON_OPS(CREATE_TABLE_ENTRY)
            INE_VM_NUMERIC_OPS(CREATE_TABLE_ENTRY)
            INE_VM_CONVERSION_OPS(CREATE_TABLE_ENTRY)
            INE_VM_EXIT_OP(CREATE_TABLE_ENTRY)
            INE_VM_EMPTY_OPS(CREATE_TABLE_ENTRY)
            INE_VM_ERROR_OPS(CREATE_TABLE_ENTRY)
            &&__ev_last
         };
         auto* ev_variant = _state.pc;
         goto *dispatch_table[ev_variant->index()];
         while (1) {
             INE_VM_CONTROL_FLOW_OPS(CREATE_LABEL);
             INE_VM_BR_TABLE_OP(CREATE_LABEL);
             INE_VM_RETURN_OP(CREATE_LABEL);
             INE_VM_CALL_OPS(CREATE_LABEL);
             INE_VM_CALL_IMM_OPS(CREATE_LABEL);
             INE_VM_PAMEMETRIC_OPS(CREATE_LABEL);
             INE_VM_VARIABLE_ACCESS_OPS(CREATE_LABEL);
             INE_VM_MEMORY_OPS(CREATE_LABEL);
             INE_VM_I32_CONSTANT_OPS(CREATE_LABEL);
             INE_VM_I64_CONSTANT_OPS(CREATE_LABEL);
             INE_VM_F32_CONSTANT_OPS(CREATE_LABEL);
             INE_VM_F64_CONSTANT_OPS(CREATE_LABEL);
             INE_VM_COMPARISON_OPS(CREATE_LABEL);
             INE_VM_NUMERIC_OPS(CREATE_LABEL);
             INE_VM_CONVERSION_OPS(CREATE_LABEL);
             INE_VM_EXIT_OP(CREATE_EXIT_LABEL);
             INE_VM_EMPTY_OPS(CREATE_EMPTY_LABEL);
             INE_VM_ERROR_OPS(CREATE_LABEL);
             __ev_last:

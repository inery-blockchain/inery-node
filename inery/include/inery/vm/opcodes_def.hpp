...
/* clang-format off */
#define INE_VM_CONTROL_FLOW_OPS(opcode_macro)   \
...
#define INE_VM_BR_TABLE_OP(opcode_macro)        \
   opcode_macro(br_table, 0x0E)
#define INE_VM_RETURN_OP(opcode_macro)          \
   opcode_macro(return_, 0x0F)
#define INE_VM_CALL_OPS(opcode_macro)           \
   opcode_macro(call, 0x10)                     \
   opcode_macro(call_indirect, 0x11)            \
   opcode_macro(br_table_data, 0x12)
#define INE_VM_CALL_IMM_OPS(opcode_macro)       \
...
#define INE_VM_PAMEMETRIC_OPS(opcode_macro)     \
...
#define INE_VM_VARIABLE_ACCESS_OPS(opcode_macro)\
...
#define INE_VM_MEMORY_OPS(opcode_macro)         \
...
#define INE_VM_I32_CONSTANT_OPS(opcode_macro)   \
   opcode_macro(i32_const, 0x41)
#define INE_VM_I64_CONSTANT_OPS(opcode_macro)   \
   opcode_macro(i64_const, 0x42)
#define INE_VM_F32_CONSTANT_OPS(opcode_macro)   \
   opcode_macro(f32_const, 0x43)
#define INE_VM_F64_CONSTANT_OPS(opcode_macro)   \
   opcode_macro(f64_const, 0x44)
#define INE_VM_COMPARISON_OPS(opcode_macro)     \
...
#define INE_VM_NUMERIC_OPS(opcode_macro)        \
...
#define INE_VM_CONVERSION_OPS(opcode_macro)     \
...
#define INE_VM_EXIT_OP(opcode_macro)            \
   opcode_macro(exit, 0xC0)
#define INE_VM_EMPTY_OPS(opcode_macro)          \
...
#define INE_VM_ERROR_OPS(opcode_macro)          \
   opcode_macro(error, 0xFF)

/* clang-format on */

#define INE_VM_CREATE_ENUM(name, code) name = code,

#define INE_VM_CREATE_STRINGS(name, code) #name,

#define INE_VM_CREATE_MAP(name, code) { code, #name },

#define INE_VM_OPCODE_NAME_if_
#define INE_VM_OPCODE_NAME_else_
#define INE_VM_OPCODE_NAME_return_

#define INE_VM_OPCODE_NAME_TEST() 1
#define INE_VM_OPCODE_NAME_TEST_INE_VM_OPCODE_NAME_TEST 0,
#define INE_VM_OPCODE_NAME_TEST_1 1, ignore
#define INE_VM_EXPAND(x) x
#define INE_VM_CAT2(x, y) x ## y
#define INE_VM_CAT(x, y) INE_VM_CAT2(x, y)
#define INE_VM_APPLY(f, args) f args
#define INE_VM_FIX_OPCODE_NAME_0(name) name ## _t
#define INE_VM_FIX_OPCODE_NAME_1(name) name ## t
#define INE_VM_FIX_OPCODE_NAME(iskeyword, garbage) INE_VM_FIX_OPCODE_NAME_ ## iskeyword

#define INE_VM_OPCODE_T(name)                                                             \
   INE_VM_APPLY(INE_VM_FIX_OPCODE_NAME,                                                   \
      (INE_VM_CAT(INE_VM_OPCODE_NAME_TEST_,                                               \
           INE_VM_EXPAND(INE_VM_OPCODE_NAME_TEST INE_VM_OPCODE_NAME_ ## name ()))))(name)

#define INE_VM_CREATE_EXIT_TYPE(name, code)                                                                      \
   struct INE_VM_OPCODE_T(name) {                                                                                      \
      INE_VM_OPCODE_T(name)() = default;                                                                               \
      uint32_t pc;                                                                                                     \
      static constexpr uint8_t opcode = code;                                                                          \
   };

#define INE_VM_CREATE_CONTROL_FLOW_TYPES(name, code)                                                                   \
   struct INE_VM_OPCODE_T(name) {                                                                                      \
      INE_VM_OPCODE_T(name)() {}                                                                                       \
      INE_VM_OPCODE_T(name)(uint32_t data) : data(data) {}                                                             \
      INE_VM_OPCODE_T(name)(uint32_t d, uint32_t pc, uint16_t i, uint16_t oi)                                          \
...
#define INE_VM_CREATE_BR_TABLE_TYPE(name, code)                                                                        \
   struct INE_VM_OPCODE_T(name) {                                                                                      \
      INE_VM_OPCODE_T(name)() = default;                                                                               \
...
#define INE_VM_CREATE_TYPES(name, code)                                                                                \
   struct INE_VM_OPCODE_T(name) {                                                                                      \
      INE_VM_OPCODE_T(name)() = default;                                                                               \
      static constexpr uint8_t opcode = code;                                                                          \
   };

#define INE_VM_CREATE_CALL_TYPES(name, code)                                                                           \
   struct INE_VM_OPCODE_T(name) {                                                                                      \
      INE_VM_OPCODE_T(name)() = default;                                                                               \
      uint32_t index;                                                                                                  \
      static constexpr uint8_t opcode = code;                                                                          \
...
#define INE_VM_CREATE_CALL_IMM_TYPES(name, code)                                                                       \
   struct INE_VM_OPCODE_T(name) {                                                                                      \
      INE_VM_OPCODE_T(name)() = default;                                                                               \
      uint32_t index;                                                                                                  \
      uint16_t locals;                                                                                                 \
...
#define INE_VM_CREATE_VARIABLE_ACCESS_TYPES(name, code)                                                                \
   struct INE_VM_OPCODE_T(name) {                                                                                      \
      INE_VM_OPCODE_T(name)() = default;                                                                               \
      uint32_t index;                                                                                                  \
      static constexpr uint8_t opcode = code;                                                                          \
...
#define INE_VM_CREATE_MEMORY_TYPES(name, code)                                                                         \
   struct INE_VM_OPCODE_T(name) {                                                                                      \
      INE_VM_OPCODE_T(name)() = default;                                                                               \
...
#define INE_VM_CREATE_I32_CONSTANT_TYPE(name, code)                                                                    \
   struct INE_VM_OPCODE_T(name) {                                                                                      \
      INE_VM_OPCODE_T(name)() = default;                                                                               \
      explicit INE_VM_OPCODE_T(name)(uint32_t n) { data.ui = n; }                                                      \
      explicit INE_VM_OPCODE_T(name)(int32_t n) { data.i = n; }                                                        \
      union {                                                                                                          \
         uint32_t ui;                                                                                                  \
         int32_t  i;                                                                                                   \
      } data;                                                                                                          \
...
#define INE_VM_CREATE_I64_CONSTANT_TYPE(name, code)                                                                    \
   struct INE_VM_OPCODE_T(name) {                                                                                      \
      INE_VM_OPCODE_T(name)() = default;                                                                               \
      explicit INE_VM_OPCODE_T(name)(uint64_t n) { data.ui = n; }                                                      \
      explicit INE_VM_OPCODE_T(name)(int64_t n) { data.i = n; }                                                        \
      union {                                                                                                          \
         uint64_t ui;                                                                                                  \
         int64_t  i;                                                                                                   \
      } data;                                                                                                          \
...
#define INE_VM_CREATE_F32_CONSTANT_TYPE(name, code)                                                                    \
   struct INE_VM_OPCODE_T(name) {                                                                                      \
      INE_VM_OPCODE_T(name)() = default;                                                                               \
      explicit INE_VM_OPCODE_T(name)(uint32_t n) { data.ui = n; }                                                      \
      explicit INE_VM_OPCODE_T(name)(float n) { data.f = n; }                                                          \
      union {                                                                                                          \
         uint32_t ui;                                                                                                  \
         float    f;                                                                                                   \
      } data;                                                                                                          \
...
#define INE_VM_CREATE_F64_CONSTANT_TYPE(name, code)                                                                    \
   struct INE_VM_OPCODE_T(name) {                                                                                      \
      INE_VM_OPCODE_T(name)() = default;                                                                               \
      explicit INE_VM_OPCODE_T(name)(uint64_t n) { data.ui = n; }                                                      \
      explicit INE_VM_OPCODE_T(name)(double n) { data.f = n; }                                                         \
      union {                                                                                                          \
         uint64_t ui;                                                                                                  \
         double   f;                                                                                                   \
      } data;                                                                                                          \
...
#define INE_VM_IDENTITY(name, code) inery::vm::INE_VM_OPCODE_T(name),
#define INE_VM_IDENTITY_END(name, code) inery::vm::INE_VM_OPCODE_T(name)

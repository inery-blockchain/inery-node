...
namespace inery { namespace vm {
   ...
   struct module {
      ...
      inline opcode* get_function_pc( uint32_t fidx ) const {
         INE_VM_ASSERT( fidx >= get_imported_functions_size(), wasm_interpreter_exception, "trying to get the PC of an imported function" );
      ...
      inline uint32_t get_function_locals_size(uint32_t index) const {
         INE_VM_ASSERT(index >= get_imported_functions_size(), wasm_interpreter_exception, "imported functions do not have locals");

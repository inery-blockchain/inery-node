...
#include <inery/vm/config.hpp>
#include <inery/vm/base_visitor.hpp>
#include <inery/vm/exceptions.hpp>
#include <inery/vm/opcodes.hpp>
#include <inery/vm/softfloat.hpp>
#include <inery/vm/stack_elem.hpp>
#include <inery/vm/utils.hpp>
#include <inery/vm/wasm_stack.hpp>
...
namespace inery { namespace vm {
   ...
   struct interpret_visitor : base_visitor {
      ...
      [[gnu::always_inline]] inline void operator()(const call_indirect_t& op) {
         ...
         INE_VM_ASSERT(actual_type == expected_type, wasm_interpreter_exception, "bad call_indirect type");
         context.call(fn);
      }
      ...
      [[gnu::always_inline]] inline void operator()(const f32_eq_t& op) {
         ...
         if constexpr (use_softfloat)
            lhs = i32_const_t{ (uint32_t)_inery_f32_eq(lhs.to_f32(), rhs) };
         else
      ...
      [[gnu::always_inline]] inline void operator()(const f32_ne_t& op) {
         ...
         auto&       lhs = context.peek_operand();
         if constexpr (use_softfloat)
            lhs = i32_const_t{ (uint32_t)_inery_f32_ne(lhs.to_f32(), rhs) };
         else
            lhs = i32_const_t{ (uint32_t)(lhs.to_f32() != rhs) };
      }
      [[gnu::always_inline]] inline void operator()(const f32_lt_t& op) {
         ...
         auto&       lhs = context.peek_operand();
         if constexpr (use_softfloat)
            lhs = i32_const_t{ (uint32_t)_inery_f32_lt(lhs.to_f32(), rhs) };
         else
            lhs = i32_const_t{ (uint32_t)(lhs.to_f32() < rhs) };
      }
      [[gnu::always_inline]] inline void operator()(const f32_gt_t& op) {
         ...
         auto&       lhs = context.peek_operand();
         if constexpr (use_softfloat)
            lhs = i32_const_t{ (uint32_t)_inery_f32_gt(lhs.to_f32(), rhs) };
         else
            lhs = i32_const_t{ (uint32_t)(lhs.to_f32() > rhs) };
      }
      [[gnu::always_inline]] inline void operator()(const f32_le_t& op) {
         ...
         auto&       lhs = context.peek_operand();
         if constexpr (use_softfloat)
            lhs = i32_const_t{ (uint32_t)_inery_f32_le(lhs.to_f32(), rhs) };
         else
            lhs = i32_const_t{ (uint32_t)(lhs.to_f32() <= rhs) };
      }
      [[gnu::always_inline]] inline void operator()(const f32_ge_t& op) {
         ...
         auto&       lhs = context.peek_operand();
         if constexpr (use_softfloat)
            lhs = i32_const_t{ (uint32_t)_inery_f32_ge(lhs.to_f32(), rhs) };
         else
            lhs = i32_const_t{ (uint32_t)(lhs.to_f32() >= rhs) };
      }
      [[gnu::always_inline]] inline void operator()(const f64_eq_t& op) {
         ...
         auto&       lhs = context.peek_operand();
         if constexpr (use_softfloat)
            lhs = i32_const_t{ (uint32_t)_inery_f64_eq(lhs.to_f64(), rhs) };
         else
      ...
      [[gnu::always_inline]] inline void operator()(const f64_ne_t& op) {
         ...
         auto&       lhs = context.peek_operand();
         if constexpr (use_softfloat)
            lhs = i32_const_t{ (uint32_t)_inery_f64_ne(lhs.to_f64(), rhs) };
         else
            lhs = i32_const_t{ (uint32_t)(lhs.to_f64() != rhs) };
      }
      [[gnu::always_inline]] inline void operator()(const f64_lt_t& op) {
         ...
         auto&       lhs = context.peek_operand();
         if constexpr (use_softfloat)
            lhs = i32_const_t{ (uint32_t)_inery_f64_lt(lhs.to_f64(), rhs) };
         else
            lhs = i32_const_t{ (uint32_t)(lhs.to_f64() < rhs) };
      }
      [[gnu::always_inline]] inline void operator()(const f64_gt_t& op) {
         ...
         auto&       lhs = context.peek_operand();
         if constexpr (use_softfloat)
            lhs = i32_const_t{ (uint32_t)_inery_f64_gt(lhs.to_f64(), rhs) };
         else
            lhs = i32_const_t{ (uint32_t)(lhs.to_f64() > rhs) };
      }
      [[gnu::always_inline]] inline void operator()(const f64_le_t& op) {
         ...
         auto&       lhs = context.peek_operand();
         if constexpr (use_softfloat)
            lhs = i32_const_t{ (uint32_t)_inery_f64_le(lhs.to_f64(), rhs) };
         else
            lhs = i32_const_t{ (uint32_t)(lhs.to_f64() <= rhs) };
      }
      [[gnu::always_inline]] inline void operator()(const f64_ge_t& op) {
         ...
         auto&       lhs = context.peek_operand();
         if constexpr (use_softfloat)
            lhs = i32_const_t{ (uint32_t)_inery_f64_ge(lhs.to_f64(), rhs) };
         else
            lhs = i32_const_t{ (uint32_t)(lhs.to_f64() >= rhs) };
      }
      ...
      [[gnu::always_inline]] inline void operator()(const i32_div_s_t& op) {
         ...
         const auto& rhs = context.pop_operand().to_i32();
         auto&       lhs = context.peek_operand().to_i32();
         INE_VM_ASSERT(rhs != 0, wasm_interpreter_exception, "i32.div_s divide by zero");
         INE_VM_ASSERT(!(lhs == std::numeric_limits<int32_t>::min() && rhs == -1), wasm_interpreter_exception,
      ...
      [[gnu::always_inline]] inline void operator()(const i32_div_u_t& op) {
         ...
         const auto& rhs = context.pop_operand().to_ui32();
         auto&       lhs = context.peek_operand().to_ui32();
         INE_VM_ASSERT(rhs != 0, wasm_interpreter_exception, "i32.div_u divide by zero");
         lhs /= rhs;
      }
      [[gnu::always_inline]] inline void operator()(const i32_rem_s_t& op) {
         ...
         const auto& rhs = context.pop_operand().to_i32();
         auto&       lhs = context.peek_operand().to_i32();
         INE_VM_ASSERT(rhs != 0, wasm_interpreter_exception, "i32.rem_s divide by zero");
      ...
      [[gnu::always_inline]] inline void operator()(const i32_rem_u_t& op) {
         ...
         const auto& rhs = context.pop_operand().to_ui32();
         auto&       lhs = context.peek_operand().to_ui32();
         INE_VM_ASSERT(rhs != 0, wasm_interpreter_exception, "i32.rem_u divide by zero");
         lhs %= rhs;
      }
      ...
      [[gnu::always_inline]] inline void operator()(const i64_div_s_t& op) {
         ...
         INE_VM_ASSERT(rhs != 0, wasm_interpreter_exception, "i64.div_s divide by zero");
         INE_VM_ASSERT(!(lhs == std::numeric_limits<int64_t>::min() && rhs == -1), wasm_interpreter_exception,
      ...
      [[gnu::always_inline]] inline void operator()(const i64_div_u_t& op) {
         ...
         const auto& rhs = context.pop_operand().to_ui64();
         auto&       lhs = context.peek_operand().to_ui64();
         INE_VM_ASSERT(rhs != 0, wasm_interpreter_exception, "i64.div_u divide by zero");
         lhs /= rhs;
      }
      [[gnu::always_inline]] inline void operator()(const i64_rem_s_t& op) {
         ...
         const auto& rhs = context.pop_operand().to_i64();
         auto&       lhs = context.peek_operand().to_i64();
         INE_VM_ASSERT(rhs != 0, wasm_interpreter_exception, "i64.rem_s divide by zero");
      ...
      [[gnu::always_inline]] inline void operator()(const f32_add_t& op) {
         ...
         if constexpr (use_softfloat)
            lhs = _inery_f32_add(lhs, rhs.to_f32());
         else
            lhs += rhs.to_f32();
      }
      [[gnu::always_inline]] inline void operator()(const f32_sub_t& op) {
         ...
         auto&       lhs = context.peek_operand().to_f32();
         if constexpr (use_softfloat)
            lhs = _inery_f32_sub(lhs, rhs.to_f32());
         else
            lhs -= rhs.to_f32();
      }
      [[gnu::always_inline]] inline void operator()(const f32_mul_t& op) {
         ...
         auto&       lhs = context.peek_operand().to_f32();
         if constexpr (use_softfloat) {
            lhs = _inery_f32_mul(lhs, rhs.to_f32());
         } else
            lhs *= rhs.to_f32();
      }
      [[gnu::always_inline]] inline void operator()(const f32_div_t& op) {
         ...
         auto&       lhs = context.peek_operand().to_f32();
         if constexpr (use_softfloat)
            lhs = _inery_f32_div(lhs, rhs.to_f32());
         else
            lhs /= rhs.to_f32();
      }
      [[gnu::always_inline]] inline void operator()(const f32_min_t& op) {
         ...
         auto&       lhs = context.peek_operand().to_f32();
         if constexpr (use_softfloat)
            lhs = _inery_f32_min(lhs, rhs.to_f32());
         else
      ...
      [[gnu::always_inline]] inline void operator()(const f32_max_t& op) {
         ...
         auto&       lhs = context.peek_operand().to_f32();
         if constexpr (use_softfloat)
            lhs = _inery_f32_max(lhs, rhs.to_f32());
         else
      ...
      [[gnu::always_inline]] inline void operator()(const f32_copysign_t& op) {
         ...
         auto&       lhs = context.peek_operand().to_f32();
         if constexpr (use_softfloat)
            lhs = _inery_f32_copysign(lhs, rhs.to_f32());
         else
      ...
      [[gnu::always_inline]] inline void operator()(const f64_add_t& op) {
         ...
         if constexpr (use_softfloat)
            lhs = _inery_f64_add(lhs, rhs.to_f64());
         else
            lhs += rhs.to_f64();
      }
      [[gnu::always_inline]] inline void operator()(const f64_sub_t& op) {
         ...
         auto&       lhs = context.peek_operand().to_f64();
         if constexpr (use_softfloat)
            lhs = _inery_f64_sub(lhs, rhs.to_f64());
         else
            lhs -= rhs.to_f64();
      }
      [[gnu::always_inline]] inline void operator()(const f64_mul_t& op) {
         ...
         auto&       lhs = context.peek_operand().to_f64();
         if constexpr (use_softfloat)
            lhs = _inery_f64_mul(lhs, rhs.to_f64());
         else
            lhs *= rhs.to_f64();
      }
      [[gnu::always_inline]] inline void operator()(const f64_div_t& op) {
         ...
         auto&       lhs = context.peek_operand().to_f64();
         if constexpr (use_softfloat)
            lhs = _inery_f64_div(lhs, rhs.to_f64());
         else
            lhs /= rhs.to_f64();
      }
      [[gnu::always_inline]] inline void operator()(const f64_min_t& op) {
         ...
         auto&       lhs = context.peek_operand().to_f64();
         if constexpr (use_softfloat)
            lhs = _inery_f64_min(lhs, rhs.to_f64());
         else
      ...
      [[gnu::always_inline]] inline void operator()(const f64_max_t& op) {
         ...
         auto&       lhs = context.peek_operand().to_f64();
         if constexpr (use_softfloat)
            lhs = _inery_f64_max(lhs, rhs.to_f64());
         else
      ...
      [[gnu::always_inline]] inline void operator()(const f64_copysign_t& op) {
         ...
         auto&       lhs = context.peek_operand().to_f64();
         if constexpr (use_softfloat)
            lhs = _inery_f64_copysign(lhs, rhs.to_f64());
         else
      ...
      [[gnu::always_inline]] inline void operator()(const i32_trunc_s_f32_t& op) {
         ...
         auto& oper = context.peek_operand();
         if constexpr (use_softfloat) {
            oper = i32_const_t{ _inery_f32_trunc_i32s(oper.to_f32()) };
         } else {
            float af = oper.to_f32();
            INE_VM_ASSERT(!((af >= 2147483648.0f) || (af < -2147483648.0f)), wasm_interpreter_exception, "Error, f32.trunc_s/i32 overflow" );
            INE_VM_ASSERT(!__builtin_isnan(af), wasm_interpreter_exception, "Error, f32.trunc_s/i32 unrepresentable");
      ...
      [[gnu::always_inline]] inline void operator()(const i32_trunc_u_f32_t& op) {
         ...
         auto& oper = context.peek_operand();
         if constexpr (use_softfloat) {
            oper = i32_const_t{ _inery_f32_trunc_i32u(oper.to_f32()) };
         } else {
            float af = oper.to_f32();
            INE_VM_ASSERT(!((af >= 4294967296.0f) || (af <= -1.0f)),wasm_interpreter_exception, "Error, f32.trunc_u/i32 overflow");
            INE_VM_ASSERT(!__builtin_isnan(af), wasm_interpreter_exception, "Error, f32.trunc_u/i32 unrepresentable");
      ...
      [[gnu::always_inline]] inline void operator()(const i32_trunc_s_f64_t& op) {
         ...
         auto& oper = context.peek_operand();
         if constexpr (use_softfloat) {
            oper = i32_const_t{ _inery_f64_trunc_i32s(oper.to_f64()) };
         } else {
            double af = oper.to_f64();
            INE_VM_ASSERT(!((af >= 2147483648.0) || (af < -2147483648.0)), wasm_interpreter_exception, "Error, f64.trunc_s/i32 overflow");
            INE_VM_ASSERT(!__builtin_isnan(af), wasm_interpreter_exception, "Error, f64.trunc_s/i32 unrepresentable");
            oper = i32_const_t{ static_cast<int32_t>(af) };
         }
      }
      [[gnu::always_inline]] inline void operator()(const i32_trunc_u_f64_t& op) {
         ...
         auto& oper = context.peek_operand();
         if constexpr (use_softfloat) {
            oper = i32_const_t{ _inery_f64_trunc_i32u(oper.to_f64()) };
         } else {
            double af = oper.to_f64();
            INE_VM_ASSERT(!((af >= 4294967296.0) || (af <= -1.0)), wasm_interpreter_exception, "Error, f64.trunc_u/i32 overflow");
            INE_VM_ASSERT(!__builtin_isnan(af), wasm_interpreter_exception, "Error, f64.trunc_u/i32 unrepresentable");
            oper = i32_const_t{ static_cast<uint32_t>(af) };
         }
      }
      ...
      [[gnu::always_inline]] inline void operator()(const i64_trunc_s_f32_t& op) {
         ...
         auto& oper = context.peek_operand();
         if constexpr (use_softfloat) {
            oper = i64_const_t{ _inery_f32_trunc_i64s(oper.to_f32()) };
         } else {
            float af = oper.to_f32();
            INE_VM_ASSERT(!((af >= 9223372036854775808.0f) || (af < -9223372036854775808.0f)), wasm_interpreter_exception, "Error, f32.trunc_s/i64 overflow");
            INE_VM_ASSERT(!__builtin_isnan(af), wasm_interpreter_exception, "Error, f32.trunc_s/i64 unrepresentable");
      ...
      [[gnu::always_inline]] inline void operator()(const i64_trunc_u_f32_t& op) {
         ...
         auto& oper = context.peek_operand();
         if constexpr (use_softfloat) {
            oper = i64_const_t{ _inery_f32_trunc_i64u(oper.to_f32()) };
         } else {
            float af = oper.to_f32();
            INE_VM_ASSERT(!((af >= 18446744073709551616.0f) || (af <= -1.0f)), wasm_interpreter_exception, "Error, f32.trunc_u/i64 overflow");
            INE_VM_ASSERT(!__builtin_isnan(af), wasm_interpreter_exception, "Error, f32.trunc_u/i64 unrepresentable");
      ...
      [[gnu::always_inline]] inline void operator()(const i64_trunc_s_f64_t& op) {
         ...
         auto& oper = context.peek_operand();
         if constexpr (use_softfloat) {
            oper = i64_const_t{ _inery_f64_trunc_i64s(oper.to_f64()) };
         } else {
            double af = oper.to_f64();
            INE_VM_ASSERT(!((af >= 9223372036854775808.0) || (af < -9223372036854775808.0)), wasm_interpreter_exception, "Error, f64.trunc_s/i64 overflow");
            INE_VM_ASSERT(!__builtin_isnan(af), wasm_interpreter_exception, "Error, f64.trunc_s/i64 unrepresentable");
            oper = i64_const_t{ static_cast<int64_t>(af) };
         }
      }
      [[gnu::always_inline]] inline void operator()(const i64_trunc_u_f64_t& op) {
         ...
         auto& oper = context.peek_operand();
         if constexpr (use_softfloat) {
            oper = i64_const_t{ _inery_f64_trunc_i64u(oper.to_f64()) };
         } else {
            double af = oper.to_f64();
            INE_VM_ASSERT(!((af >= 18446744073709551616.0) || (af <= -1.0)), wasm_interpreter_exception, "Error, f64.trunc_u/i64 overflow");
            INE_VM_ASSERT(!__builtin_isnan(af), wasm_interpreter_exception, "Error, f64.trunc_u/i64 unrepresentable");
            oper = i64_const_t{ static_cast<uint64_t>(af) };
         }
      }
      [[gnu::always_inline]] inline void operator()(const f32_convert_s_i32_t& op) {
         ...
         auto& oper = context.peek_operand();
         if constexpr (use_softfloat) {
            oper = f32_const_t{ _inery_i32_to_f32(oper.to_i32()) };
         } else {
      ...
      [[gnu::always_inline]] inline void operator()(const f32_convert_u_i32_t& op) {
         ...
         auto& oper = context.peek_operand();
         if constexpr (use_softfloat) {
            oper = f32_const_t{ _inery_ui32_to_f32(oper.to_ui32()) };
         } else {
      ...
      [[gnu::always_inline]] inline void operator()(const f32_convert_s_i64_t& op) {
         ...
         auto& oper = context.peek_operand();
         if constexpr (use_softfloat) {
            oper = f32_const_t{ _inery_i64_to_f32(oper.to_i64()) };
         } else {
      ...
      [[gnu::always_inline]] inline void operator()(const f32_convert_u_i64_t& op) {
         ...
         auto& oper = context.peek_operand();
         if constexpr (use_softfloat) {
            oper = f32_const_t{ _inery_ui64_to_f32(oper.to_ui64()) };
         } else {
      ...
      [[gnu::always_inline]] inline void operator()(const f32_demote_f64_t& op) {
         ...
         auto& oper = context.peek_operand();
         if constexpr (use_softfloat) {
            oper = f32_const_t{ _inery_f64_demote(oper.to_f64()) };
         } else {
      ...
      [[gnu::always_inline]] inline void operator()(const f64_convert_s_i32_t& op) {
         ...
         auto& oper = context.peek_operand();
         if constexpr (use_softfloat) {
            oper = f64_const_t{ _inery_i32_to_f64(oper.to_i32()) };
         } else {
      ...
      [[gnu::always_inline]] inline void operator()(const f64_convert_u_i32_t& op) {
         ...
         auto& oper = context.peek_operand();
         if constexpr (use_softfloat) {
            oper = f64_const_t{ _inery_ui32_to_f64(oper.to_ui32()) };
         } else {
      ...
      [[gnu::always_inline]] inline void operator()(const f64_convert_s_i64_t& op) {
         ...
         auto& oper = context.peek_operand();
         if constexpr (use_softfloat) {
            oper = f64_const_t{ _inery_i64_to_f64(oper.to_i64()) };
         } else {
      ...
      [[gnu::always_inline]] inline void operator()(const f64_convert_u_i64_t& op) {
         ...
         auto& oper = context.peek_operand();
         if constexpr (use_softfloat) {
            oper = f64_const_t{ _inery_ui64_to_f64(oper.to_ui64()) };
         } else {
      ...
      [[gnu::always_inline]] inline void operator()(const f64_promote_f32_t& op) {
         ...
         auto& oper = context.peek_operand();
         if constexpr (use_softfloat) {
            oper = f64_const_t{ _inery_f32_promote(oper.to_f32()) };
         } else {

...
namespace inery { namespace chain {
   ...
   class controller {
      public:
         ...
         bool is_mem_billing_in_notify_allowed()const;

         //This is only an accessor to the user configured subjective limit: i.e. it does not do a
         // check similar to is_mem_billing_in_notify_allowed() to check if controller is currently
         // producing a block
         ...
         void add_to_mem_correction( account_name account, uint64_t mem_bytes );

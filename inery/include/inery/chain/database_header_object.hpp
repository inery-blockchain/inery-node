...
namespace inery { namespace chain {
      ...
      class database_header_object : public chainbase::object<database_header_object_type, database_header_object>
      {
          ...
          *         change from master_key to master_authority for many in-memory structures
          */
         ...
         void validate() const {
            INE_ASSERT(std::clamp(version, minimum_version, current_version) == version, bad_database_version_exception,
...
CHAINBASE_SET_INDEX_TYPE(inery::chain::database_header_object, inery::chain::database_header_multi_index)

FC_REFLECT( inery::chain::database_header_object, (version) )

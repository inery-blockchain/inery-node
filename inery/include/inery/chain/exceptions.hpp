...
#define INE_ASSERT( expr, exc_type, FORMAT, ... )                \
   FC_MULTILINE_MACRO_BEGIN                                           \
   if( !(expr) )                                                      \
      FC_THROW_EXCEPTION( exc_type, FORMAT, __VA_ARGS__ );            \
   FC_MULTILINE_MACRO_END

#define INE_THROW( exc_type, FORMAT, ... ) \
    throw exc_type( FC_LOG_MESSAGE( error, FORMAT, __VA_ARGS__ ) );

/**
 * Macro inspired from FC_RETHROW_EXCEPTIONS
 * The main difference here is that if the exception caught isn't of type "inery::chain::chain_exception"
 * This macro will rethrow the exception as the specified "exception_type"
 */
#define INE_RETHROW_EXCEPTIONS(exception_type, FORMAT, ... ) \
   catch( const std::bad_alloc& ) {\
      throw;\
   } catch( const boost::interprocess::bad_alloc& ) {\
      throw;\
   } catch (inery::chain::chain_exception& e) { \
 ...
 * This macro will rethrow the exception as the specified "exception_type"
 */
#define INE_CAPTURE_AND_RETHROW( exception_type, ... ) \
   catch( const std::bad_alloc& ) {\
      throw;\
   } catch( const boost::interprocess::bad_alloc& ) {\
      throw;\
   } catch (inery::chain::chain_exception& e) { \
      FC_RETHROW_EXCEPTION( e, warn, "", FC_FORMAT_ARG_PAMEMS(__VA_ARGS__) ); \
   } catch (fc::exception& e) { \
      exception_type new_exception(e.get_log()); \
      throw new_exception; \
   } catch( const std::exception& e ) {  \
      exception_type fce( \
                FC_LOG_MESSAGE( warn, "${what}: ",FC_FORMAT_ARG_PAMEMS(__VA_ARGS__)("what",e.what())), \
                fc::std_exception_code,\
                BOOST_CORE_TYPEID(decltype(e)).name(), \
                e.what() ) ; throw fce;\
   } catch( ... ) {  \
      throw fc::unhandled_exception( \
                FC_LOG_MESSAGE( warn, "",FC_FORMAT_ARG_PAMEMS(__VA_ARGS__)), \
                std::current_exception() ); \
   }
...
#define INE_RECODE_EXC( cause_type, effect_type ) \
...
namespace inery { namespace chain {
   ...
   FC_DECLARE_DERIVED_EXCEPTION( action_validate_exception, chain_exception,
      ...
      FC_DECLARE_DERIVED_EXCEPTION( inery_assert_message_exception, action_validate_exception,
                                    3050003, "inery_assert_message assertion failure" )
      FC_DECLARE_DERIVED_EXCEPTION( inery_assert_code_exception, action_validate_exception,
                                    3050004, "inery_assert_code assertion failure" )

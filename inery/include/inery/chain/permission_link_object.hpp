...
namespace inery { namespace chain {
   ...
   namespace config {
      ...
      struct billable_size<permission_link_object> {
         static const uint64_t overhead = overhead_per_row_per_index_mem_bytes * 3; ///< 3x indices id, action, permission
         static const uint64_t value = 40 + overhead; ///< fixed field + overhead
      };
   }
} } // inery::chain

CHAINBASE_SET_INDEX_TYPE(inery::chain::permission_link_object, inery::chain::permission_link_index)

FC_REFLECT(inery::chain::permission_link_object, (account)(code)(message_type)(required_permission))

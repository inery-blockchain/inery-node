...
namespace inery { namespace chain {
   ...
   namespace config {
      ...
      struct billable_size<permission_object> { // Also counts memory usage of the associated permission_usage_object
         static const uint64_t  overhead = 5 * overhead_per_row_per_index_mem_bytes; ///< 5 indices 2x internal ID, parent, owner, name
         static const uint64_t  value = (config::billable_size_v<shared_authority> + 64) + overhead;  ///< fixed field size + overhead
      };
   }
} } // inery::chain

CHAINBASE_SET_INDEX_TYPE(inery::chain::permission_object, inery::chain::permission_index)
CHAINBASE_SET_INDEX_TYPE(inery::chain::permission_usage_object, inery::chain::permission_usage_index)

FC_REFLECT(inery::chain::permission_object, (usage_id)(parent)(owner)(name)(last_updated)(auth))
FC_REFLECT(inery::chain::snapshot_permission_object, (parent)(owner)(name)(last_updated)(last_used)(auth))

FC_REFLECT(inery::chain::permission_usage_object, (last_used))

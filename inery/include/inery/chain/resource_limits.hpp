...
namespace inery { namespace chain { namespace resource_limits {
   ...
   class resource_limits_manager {
      public:
         ...
         void add_pending_mem_usage( const account_name account, int64_t mem_delta );
         void verify_account_mem_usage( const account_name accunt )const;

         /// set_account_limits returns true if new mem_bytes limit is more restrictive than the previously set one
         bool set_account_limits( const account_name& account, int64_t mem_bytes, int64_t net_weight, int64_t cpu_weight);
         void get_account_limits( const account_name& account, int64_t& mem_bytes, int64_t& net_weight, int64_t& cpu_weight) const;
         ...
         int64_t get_account_mem_usage( const account_name& name ) const;
...
FC_REFLECT( inery::chain::resource_limits::account_resource_limit, (used)(available)(max) )
FC_REFLECT( inery::chain::resource_limits::ratio, (numerator)(denominator))
FC_REFLECT( inery::chain::resource_limits::elastic_limit_parameters, (target)(max)(periods)(max_multiplier)(contract_rate)(expand_rate))

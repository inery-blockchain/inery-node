...
struct chain_snapshot_header {
    ...
    *         - Adds new indices for: protocol_state_object and account_mem_correction_object
    *   3: Updated for v2.0.0 protocol features:
    *         - forwards compatible with version 2
    *         - WebAuthn keys
    *         - wtmsig block siganatures: the block header state changed to include master authorities and additional signatures
   ...
   void validate() const {
      ...
      INE_ASSERT(version >= min && version <= max,
...
FC_REFLECT(inery::chain::chain_snapshot_header,(version))

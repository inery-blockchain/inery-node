...
namespace inery { namespace chain { namespace config {
...
const static name system_account_name    { N(inery) };
...
const static name inery_any_name  { N(inery.any) };
const static name inery_code_name { N(inery.code) };
...
const static uint32_t   fixed_overhead_shared_vector_mem_bytes = 16; ///< overhead accounts for fixed portion of size of shared_vector field
const static uint32_t   overhead_per_row_per_index_mem_bytes = 32;    ///< overhead accounts for basic tracking structures in a row per index
const static uint32_t   overhead_per_account_mem_bytes     = 2*1024; ///< overhead accounts for basic account storage and pre-pays features like account recovery
const static uint32_t   setcode_mem_bytes_multiplier       = 10;     ///< multiplier on contract size to account for multiple copies and cached compilation
...
const static inery::chain::wasm_interface::vm_type default_wasm_runtime = inery::chain::wasm_interface::vm_type::wabt;
const static uint32_t   default_abi_serializer_max_time_us = 15*10000; ///< default deadline for abi serialization methods
...
} } } // namespace inery::chain::config

constexpr uint64_t INE_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / inery::chain::config::percent_100;
}

template<typename Number>
Number INE_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + inery::chain::config::percent_100 - inery::chain::config::percent_1)  / inery::chain::config::percent_100;
}

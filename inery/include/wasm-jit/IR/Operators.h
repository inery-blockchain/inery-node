...
namespace IR
{
	...
	#define ENUM_PAMEMETRIC_OPERATORS(visitOp) \
	...
	/*
		Possible signatures used by ENUM_NONCONTROL_NONPAMEMETRIC_OPERATORS:
		NULLARY(T) : () -> T
		UNARY(T,U) : T -> U
		BINARY(T,U) : (T,T) -> U
		LOAD(T) : i32 -> T
	...
	*/
	...
	#define ENUM_NONCONTROL_NONPAMEMETRIC_OPERATORS(visitOp) \
	...
	#define ENUM_NONCONTROL_OPERATORS(visitOp) \
		ENUM_PAMEMETRIC_OPERATORS(visitOp) \
		ENUM_NONCONTROL_NONPAMEMETRIC_OPERATORS(visitOp)
